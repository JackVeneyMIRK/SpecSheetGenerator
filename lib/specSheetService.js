...
function getTenantMeta(tenant) {
  let company = {};
  let brand = {};

  try { company = loadCompany(tenant); } catch {}
  try { brand = loadBrand(tenant); } catch {}

  const units = listUnits(tenant);
  return {
    tenant,
    units,
    unitCount: units.length,
    companyName: company?.name || tenant.toUpperCase(),
    phone: company?.phone || '',
    website: company?.website || '',
    primary: brand?.primary || '#1f2937',
    accent: brand?.accent || '#94a3b8',
  };
}

function listTenantSummaries() {
  return listTenants().map(getTenantMeta);
}
