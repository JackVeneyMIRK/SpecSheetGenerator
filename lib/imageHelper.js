...
function saveFocalCache() {
  try {
    ...
    // Cache write failures are non-fatal; PDF generation continues without persistence.
  }
}
...
/**
 * Build a high-quality photo data URI for PDF embedding.
 * We downscale oversized images with Lanczos + mild sharpening and encode
 * as high-quality 4:4:4 JPEG to reduce softness/grain in final PDFs.
 */
async function toPhotoDataUri(filePath) {
  const stat = fs.statSync(filePath);
  const cacheKey = `${PHOTO_CACHE_VERSION}:${filePath}:${stat.mtimeMs}`;
  const cached = photoDataCache.get(cacheKey);
  if (cached) return cached;

  const ext = path.extname(filePath).toLowerCase();
  const sourceMime = MIME[ext] ?? 'image/jpeg';
  const isPhoto = sourceMime !== 'image/svg+xml' && sourceMime !== 'image/gif';

  if (!isPhoto) return toDataUri(filePath);
  ...
  photoDataCache.set(cacheKey, dataUri);
  return dataUri;
}
...
async function unitPhotos(tenant, unitId) {
  ...
  return Promise.all(files.map(async f => {
    ...
    const dataUri  = await toPhotoDataUri(filePath);
    return {
      filename: f,
      dataUri,
      focalX:   focal.x,
