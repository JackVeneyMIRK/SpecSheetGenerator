...
async function getFocalPoint(filePath) {
  ...
  try {
    ...
    let bestScore = -1, bestX = 50, bestY = 60;
    ...
    for (let row = 0; row < ROWS; row++) {
      for (let col = 0; col < COLS; col++) {
        ...
        const score = stats.channels.reduce(
          (sum, ch) => sum + ch.stdev,
          0
        );
        ...
        if (score > bestScore) {
          bestScore = score;
          bestX = Math.round(((col + 0.5) / COLS) * 100);
          bestY = Math.round(((row + 0.5) / ROWS) * 100);
        }
      }
    }

    const result = { x: bestX, y: bestY };
